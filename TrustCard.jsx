...
const TrustCard = ({ status = 'Cleared' }) => {
  ...
  return (
    <div className={`relative w-full max-w-[420px] backdrop-blur-xl bg-[rgba(11,18,33,0.8)] rounded-md p-7 font-['Inter',sans-serif] transition-all duration-700 hover:-translate-y-2 ${status === 'Cleared' ? 'border border-[#00F5D4]/60 shadow-[0_15px_40px_rgba(0,245,212,0.15)] hover:shadow-[0_20px_60px_rgba(0,245,212,0.25)]' : 'border border-gray-700'}`}>
      
      {/* Dynamic Glow Behind Card if Cleared */}
      {status === 'Cleared' && (
        <div className="absolute -inset-1 bg-gradient-to-r from-[#00F5D4]/20 to-[#00F5D4]/0 rounded-md blur-xl -z-10 opacity-50"></div>
      )}
        ...
        <div>
          ...
          <p className="text-[#00F5D4] text-xs font-semibold mt-1.5 tracking-wider uppercase flex items-center gap-2">
            <span className="relative flex h-2 w-2">
              ...
              <span className="relative inline-flex rounded-md h-2 w-2 bg-[#00F5D4]"></span>
            </span>
            Verified 4 hours ago
          </p>
        </div>
        <div className={`px-3.5 py-1.5 rounded-md flex items-center gap-2 text-xs font-bold uppercase tracking-wider ${status === 'Cleared' ? 'bg-[#00F5D4]/10 text-[#00F5D4] shadow-[0_0_15px_rgba(0,245,212,0.15)] border border-[#00F5D4]/30' : 'bg-gray-800 text-gray-400'}`}>
      ...
      <div className="mb-8 relative">
        ...
        <div className="flex items-center justify-between relative px-2">
          ...
          <div className="flex flex-col items-center gap-2.5 z-10">
            <div className="w-5 h-5 rounded-md bg-[#0B1221] border-2 border-[#00F5D4] flex items-center justify-center shadow-[0_0_10px_rgba(0,245,212,0.3)]">
              <div className="w-2 h-2 rounded-md bg-[#00F5D4]"></div>
            </div>
          ...
          <div className="flex flex-col items-center gap-2.5 z-10">
            <div className="w-5 h-5 rounded-md bg-[#00F5D4] shadow-[0_0_15px_rgba(0,245,212,0.6)] flex items-center justify-center text-[#0B1221] border border-white/20">
      ...
      <button 
        ...
        className="w-full relative overflow-hidden group bg-white/[0.03] border border-white/10 rounded-md py-4 mb-6 hover:bg-white/10 transition-colors duration-300 flex items-center justify-center gap-2"
      >
      ...
      <div className={`overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] ${view3D ? 'h-[160px] opacity-100 mb-6' : 'h-0 opacity-0 mb-0'}`}>
        <div className="w-full h-full bg-[#050810] rounded-md border border-[#00F5D4]/20 relative flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center gap-2">
               <div className="w-10 h-10 rounded-md bg-[#00F5D4]/10 border border-[#00F5D4]/40 flex items-center justify-center mb-1">
               ...
               <span className="text-[#00F5D4] font-['Space_Grotesk'] text-[11px] font-bold tracking-[0.3em] bg-[#050810]/80 px-4 py-1.5 rounded-md border border-[#00F5D4]/30 backdrop-blur-sm">RENDERING ELEVATION</span>
            </div>
        </div>
      </div>

      {/* Truth-Signal: Milestone Escrow Badge Footer */}
      <div className="bg-[#050810]/60 rounded-md p-4.5 border border-[#00F5D4]/10 flex items-center gap-4 hover:border-[#00F5D4]/30 transition-colors duration-300">
