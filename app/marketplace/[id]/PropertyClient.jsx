    ...
    <div className="bg-white min-h-screen text-slate-900 font-sans flex flex-col overflow-x-hidden">
      ...
      <main className="flex-grow pt-32 pb-24 px-4 md:px-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2 mb-8 text-[10px] font-medium tracking-tight uppercase tracking-widest text-slate-400">
           ...
           <span>/</span>
           <span className="text-syntry-teal-600">{property.title}</span>
        </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          <div className="rounded-[3rem] overflow-hidden bg-slate-50 aspect-square md:aspect-video lg:aspect-square flex flex-col items-center justify-center text-center p-12 border-4 border-slate-100 relative group">
            <div className="absolute inset-0 bg-syntry-teal-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
            ...
            <div className="absolute bottom-10 left-10 right-10 flex justify-between items-center bg-white/90 backdrop-blur p-4 rounded-md border border-slate-200">
          ...
          <div className="flex flex-col justify-center space-y-8">
            <div className="flex flex-wrap gap-3">
              <span className="bg-syntry-teal-600 text-white text-[10px] font-medium tracking-tight px-4 py-2 rounded-md uppercase tracking-widest shadow-lg shadow-[#0D9488]/20">
                ✓ Sovereign Certified
              </span>
              <span className="bg-slate-900 text-white text-[10px] font-medium tracking-tight px-4 py-2 rounded-md uppercase tracking-widest">
                Mortgage Eligible
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 border-y border-slate-100 py-10">
              ...
              <div className="space-y-1 border-l border-slate-100 pl-8">
                ...
                <p className="text-4xl font-medium tracking-tight text-syntry-teal-600 tracking-tight">{property.apy}</p>
              </div>
              <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
               <button className="bg-slate-900 text-white px-10 py-6 rounded-md font-medium tracking-tight text-xs uppercase tracking-widest shadow-2xl hover:-translate-y-1 transition-all">Download Audit Report</button>
               <button className="bg-white border-2 border-slate-200 text-slate-900 px-10 py-6 rounded-md font-medium tracking-tight text-xs uppercase tracking-widest hover:border-slate-900 transition-all">Verify Coordinates</button>
            </div>
          </div>
        </section>
        ...
        <section className="mb-24">
          <div className="flex justify-between items-end mb-12">
             <div className="space-y-2">
                <p className="text-[10px] font-medium tracking-tight text-syntry-teal-600 uppercase tracking-widest">Statutory Assurance</p>
            ...
            {property.layers.map((layer, i) => (
              <div key={i} className="bg-slate-50 border border-slate-100 p-8 rounded-[2.5rem] hover:bg-white hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="w-3 h-3 bg-syntry-teal-600 rounded-md shadow-[0_0_8px_#0D9488]"></span>
                </div>
                <h4 className="font-medium tracking-tight text-slate-900 text-lg mb-2">{layer.name}</h4>
                <p className="text-[10px] font-medium tracking-tight text-syntry-teal-600 uppercase tracking-widest">{layer.status} • {layer.source}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-12 bg-slate-900 rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between gap-8 text-white">
             <div className="flex items-center gap-6">
                <div className="w-16 h-16 bg-syntry-teal-600 text-white rounded-md flex items-center justify-center text-3xl shadow-xl">⚖️</div>
                <div>
                   <p className="text-[10px] font-medium tracking-tight text-syntry-teal-600 uppercase tracking-widest mb-1">Exchange Status</p>
                   <p className="text-xl font-medium">Stamped with Statutory Finality. 0.00% Risk Index.</p>
                </div>
             </div>
             <button className="bg-white text-slate-900 px-8 py-4 rounded-md font-medium tracking-tight text-xs uppercase tracking-widest hover:scale-105 transition-all">View Registry Log</button>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-stretch">
          <div className="lg:col-span-2 bg-[#D4AF37]/5 border-4 border-[#D4AF37]/10 p-12 rounded-[4rem] flex flex-col justify-between relative overflow-hidden group">
            <div className="absolute -bottom-24 -right-24 w-96 h-96 bg-[#D4AF37]/10 rounded-md blur-[100px] group-hover:bg-[#D4AF37]/20 transition-all duration-1000"></div>
            
            <div className="space-y-6 relative z-10">
               <div className="inline-flex items-center gap-2 bg-[#D4AF37]/20 px-4 py-2 rounded-md border border-[#D4AF37]/30">
            ...
            <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-8 relative z-10">
               <div className="bg-white p-8 rounded-md shadow-xl border border-slate-100">
                  ...
                  <p className="text-[10px] font-medium tracking-tight text-syntry-teal-600 uppercase tracking-widest mt-4">✓ fits your pre-approval</p>
               </div>
               <div className="flex flex-col gap-4">
                  <a href="/mortgage" className="w-full bg-syntry-teal-600 text-white py-5 rounded-md font-medium tracking-tight text-xs text-center uppercase tracking-widest shadow-xl shadow-[#0D9488]/20 hover:-translate-y-1 transition-all">Start Mortgage App</a>
                  <a href="https://wa.me/233531102292" className="w-full bg-slate-900 text-white py-5 rounded-md font-medium tracking-tight text-xs text-center uppercase tracking-widest hover:-translate-y-1 transition-all">Talk to Underwriter</a>
               </div>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-slate-100 p-12 rounded-[4rem] shadow-xl flex flex-col justify-between">
            ...
            <div className="space-y-4 mt-12">
               <a 
                 ...
                 className="w-full bg-[#25D366] text-white py-6 rounded-md font-medium tracking-tight text-xs uppercase tracking-widest hover:scale-105 transition-all flex items-center justify-center gap-3 shadow-xl"
               >
