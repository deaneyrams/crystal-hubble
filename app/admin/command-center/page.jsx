      ...
      <div className="min-h-screen bg-[#0D1B2A] text-white flex items-center justify-center font-body p-6 text-center">
        {isScanningAdmin ? (
          <div className="text-center w-full max-w-sm relative z-10 animate-[pulse_3s_ease-in-out_infinite]">
            ...
            <h3 className="text-2xl font-head font-black text-white uppercase tracking-widest mb-2">Admin Biometric Required</h3>
          ...
          <div className="max-w-md w-full bg-[#162A3E]/80 border border-white/10 p-12 rounded-[40px] shadow-[0_50px_100px_rgba(0,0,0,0.5)] text-center relative z-10 backdrop-blur-3xl">
            ...
            <h2 className="text-3xl font-head font-black text-white uppercase tracking-tighter mb-2">Command Center</h2>
            <p className="text-[#94A3B8] font-body text-sm mb-10">Restricted Sovereign Exchange Node Protocol.</p>
            <button onClick={handleHardwareAuth} className="w-full py-5 bg-[#D4AF37] text-[#0D1B2A] font-head font-black uppercase tracking-[0.2em] text-[10px] rounded-full hover:bg-white transition-all shadow-[0_20px_40px_rgba(212,175,55,0.2)]">
    ...
    <div className="min-h-screen bg-[#0D1B2A] text-white font-body">
      <header className="sticky top-0 bg-[#0B141E]/90 backdrop-blur-xl border-b border-white/5 z-50 px-8 py-5 flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-[#162A3E] border border-white/10 rounded-xl flex items-center justify-center font-black text-[#D4AF37]">S</div>
          <div>
            <h1 className="font-head font-black uppercase tracking-widest text-sm leading-none text-white">Sovereign Admin</h1>
        ...
        <nav className="hidden md:flex bg-[#162A3E]/50 p-1 rounded-full border border-white/5 shadow-inner">
          {['inventory', 'vaults', 'pioneers', 'dev_verify', 'the_node'].map(tab => (
            <button 
              ...
              className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-[#0D1B2A] text-[#D4AF37] shadow-sm border border-white/5' : 'text-[#8CA0B4] hover:text-white'}`}
            >
              {tab.replace('_', ' ').replace('dev verify', 'Verifications')}
            </button>
          ))}
        </nav>
        <button onClick={() => setAdminAuth(false)} className="text-[#94A3B8] hover:text-white text-[10px] uppercase tracking-widest font-black transition-colors">Terminate Session</button>
      </header>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'inventory' && (
            <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="space-y-8">
              <div className="flex justify-between items-end mb-10 border-b border-white/10 pb-6">
                <div>
                  <h2 className="text-3xl font-head font-black uppercase tracking-tighter">Property Management</h2>
                  <p className="text-[#94A3B8] font-body text-sm mt-1">Deploy, verify, and orchestrate physical assets on the ledger.</p>
                </div>
                <button onClick={handleBulkWhatsApp} className="px-6 py-3 bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/50 rounded-full font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-[#10B981]/20 transition-all">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-[#162A3E] border border-white/5 rounded-[32px] p-8 shadow-xl">
                  <h3 className="font-head font-black uppercase tracking-widest text-[#D4AF37] mb-6 border-b border-white/10 pb-4">Milestone Automation</h3>
                  <div className="space-y-6">
                    <div>
                      <label className="text-[10px] font-bold uppercase tracking-widest text-white block mb-2">Target Asset</label>
                    ...
                    <div>
                      <label className="text-[10px] font-bold uppercase tracking-widest text-white block mb-2">Legal Status Workflow</label>
                    ...
                    <button onClick={handleMilestoneUpdate} className="w-full py-4 bg-gradient-to-r from-[#2DD4BF] to-[#10B981] text-[#0D1B2A] font-head font-black uppercase tracking-[0.2em] text-[10px] rounded-xl hover:scale-105 transition-all shadow-xl mt-4">
                ...
                <div className="lg:col-span-2 bg-[#162A3E]/40 border border-white/5 rounded-[32px] p-8 grid grid-cols-1 md:grid-cols-2 gap-8 relative overflow-hidden shadow-xl">
                  <div className="space-y-6 relative z-10">
                    <h3 className="font-head font-black uppercase tracking-widest text-white border-b border-white/10 pb-4">Asset Attributes</h3>
                    <div className="space-y-4">
                       <div>
                         <label className="text-[10px] font-bold uppercase tracking-widest text-white block mb-1">GPS Coordinates</label>
                       ...
                       <div>
                         <label className="text-[10px] font-bold uppercase tracking-widest text-[#2DD4BF] block mb-1">🛰️ 4K Drone Array ID</label>
                    ...
                    <div className="space-y-3 pt-4 border-t border-white/10">
                       <label className="text-[10px] font-bold uppercase tracking-widest text-white flex justify-between"><span>Litigation Clear</span> <span className="text-[#D4AF37]">{trustInputs.litigation}%</span></label>
                       ...
                       <label className="text-[10px] font-bold uppercase tracking-widest text-white flex justify-between"><span>Utilities Ready</span> <span className="text-[#10B981]">{trustInputs.utilities}%</span></label>
                  ...
                  <div className="flex flex-col items-center justify-center border-l border-white/5">
                     <div className="relative w-40 h-40 flex items-center justify-center">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                           <span className="text-3xl font-head font-black text-white">{trustScore}%</span>
                           <span className="text-[8px] uppercase font-black text-[#2DD4BF] tracking-widest">Score</span>
                        </div>
                     </div>
                  </div>
                </div>
            ...
            <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="space-y-8">
               <div className="bg-[#162A3E] border border-white/5 rounded-[32px] p-10 shadow-xl">
                  <div className="flex justify-between items-center mb-10 border-b border-white/10 pb-6">
                     <h3 className="font-head font-black uppercase tracking-widest text-[#D4AF37]">Developer Verifications</h3>
                     <p className="text-[#94A3B8] text-[10px] font-mono uppercase font-black tracking-widest">4 Node Requests Pending</p>
                  </div>
                  <div className="space-y-6">
                     {[
                        ...
                        <div key={i} className="bg-[#0D1B2A]/50 p-8 rounded-3xl border border-white/5 grid grid-cols-1 md:grid-cols-4 gap-8 items-center group hover:border-[#2DD4BF]/30 transition-all">
                           <div>
                              <p className="text-[10px] font-black uppercase text-[#94A3B8] mb-1">Entity</p>
                              <h4 className="text-white font-bold leading-tight">{app.company}</h4>
                              <p className="text-[#94A3B8] text-[9px] font-mono mt-1">{app.reg}</p>
                           </div>
                           <div>
                              <p className="text-[10px] font-black uppercase text-[#94A3B8] mb-1">Target Project</p>
                              <p className="text-white font-bold">{app.project}</p>
                           </div>
                           ...
                           <div className="flex justify-end gap-3 uppercase text-[10px] font-black tracking-widest">
             ...
             <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   <div className="bg-[#162A3E] p-10 rounded-[32px] border border-white/10 text-center">
                      <p className="text-[10px] font-black uppercase tracking-widest text-[#94A3B8] mb-2">Total Secured</p>
                      <h2 className="text-4xl font-head font-black text-white">GH₵14.2M</h2>
                   </div>
                   <div className="bg-[#162A3E] p-10 rounded-[32px] border border-[#D4AF37]/30 text-center relative overflow-hidden">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-[#94A3B8] mb-2">High-Value Scans</p>
                      <h2 className="text-4xl font-head font-black text-[#D4AF37]">04</h2>
                   </div>
                   <div className="bg-[#162A3E] p-10 rounded-[32px] border border-[#2DD4BF]/30 text-center">
                      <p className="text-[10px] font-black uppercase tracking-widest text-[#94A3B8] mb-2">Handshakes</p>
                      <h2 className="text-4xl font-head font-black text-[#2DD4BF]">1,240</h2>
                   </div>
                </div>
             </motion.div>
          )}
          ...
          {activeTab === 'pioneers' && (
             <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="bg-[#162A3E] p-10 rounded-[32px] border border-white/5">
                <h3 className="font-head font-black uppercase tracking-widest mb-10 pb-6 border-b border-white/10">Activity Ledger</h3>
                <div className="space-y-4">
                   {[
                      ...
                      <div key={i} className="flex justify-between items-center bg-[#0D1B2A]/50 p-6 rounded-2xl border border-white/5">
                         <div>
                            <span className={`text-[8px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded border mb-2 inline-block ${log.status === 'SUCCESS' ? 'text-[#10B981] border-[#10B981]/30' : 'text-[#2DD4BF] border-[#2DD4BF]/30'}`}>{log.status}</span>
                            <p className="text-white text-sm font-bold">{log.actor}</p>
          ...
          {activeTab === 'the_node' && (
             <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <h2 className="text-3xl font-head font-black uppercase tracking-tighter mb-4">System-Wide Broadcast</h2>
                <p className="text-[#94A3B8] text-sm max-w-sm mb-10">Push a global handshake to all active Sovereign client connections.</p>
                <button className="px-12 py-5 bg-[#D4AF37] text-[#0D1B2A] font-head font-black uppercase tracking-widest text-[10px] rounded-full shadow-2xl hover:scale-105 transition-all">Broadcast Handshake</button>
             </motion.div>
          )}
        </AnimatePresence>
      </main>
