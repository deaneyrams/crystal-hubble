      ...
      <div className="min-h-screen bg-[#0D1B2A] text-white flex items-center justify-center font-body p-6 text-center">
        {isScanningAdmin ? (
          <div className="text-center w-full max-w-sm relative z-10 animate-[pulse_3s_ease-in-out_infinite]">
            <div className="w-full aspect-square border-4 border-[#2DD4BF] rounded-md flex flex-col items-center justify-center relative mb-8 overflow-hidden shadow-[0_0_100px_rgba(45,212,191,0.2)] mx-auto">
          ...
          <div className="max-w-md w-full bg-[#162A3E]/80 border border-white/10 p-12 rounded-[40px] shadow-[0_50px_100px_rgba(0,0,0,0.5)] text-center relative z-10 backdrop-blur-3xl">
            <div className="w-20 h-20 bg-[#0D1B2A] rounded-md mx-auto flex items-center justify-center text-4xl mb-8 shadow-inner border border-white/5 text-[#D4AF37]">
              🛡️
            </div>
            ...
            <button onClick={handleHardwareAuth} className="w-full py-5 bg-[#D4AF37] text-[#0D1B2A] font-head font-medium tracking-tight uppercase tracking-[0.2em] text-[10px] rounded-md hover:bg-white transition-all shadow-[0_20px_40px_rgba(212,175,55,0.2)]">
    ...
    <div className="min-h-screen bg-[#0D1B2A] text-white font-body">
      <header className="sticky top-0 bg-[#0B141E]/90 backdrop-blur-xl border-b border-white/5 z-50 px-8 py-5 flex justify-between items-center">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-[#162A3E] border border-white/10 rounded-md flex items-center justify-center font-medium tracking-tight text-[#D4AF37]">S</div>
          <div>
        ...
        <nav className="hidden md:flex bg-[#162A3E]/50 p-1 rounded-md border border-white/5 shadow-inner">
          {['inventory', 'vaults', 'pioneers', 'dev_verify', 'the_node'].map(tab => (
            <button 
              key={tab} 
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-2 rounded-md text-[10px] font-medium tracking-tight uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-[#0D1B2A] text-[#D4AF37] shadow-sm border border-white/5' : 'text-[#8CA0B4] hover:text-white'}`}
            >
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'inventory' && (
            <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="space-y-8">
              <div className="flex justify-between items-end mb-10 border-b border-white/10 pb-6">
                ...
                <button onClick={handleBulkWhatsApp} className="px-6 py-3 bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/50 rounded-md font-medium tracking-tight text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-[#10B981]/20 transition-all">
                ...
                <div className="bg-[#162A3E] border border-white/5 rounded-[32px] p-8 shadow-xl">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <select className="w-full bg-[#0D1B2A] border border-white/10 text-white rounded-md py-3 px-4 outline-none focus:border-[#2DD4BF] text-xs font-mono">
                    ...
                    <div>
                      ...
                      <select value={statusAction} onChange={(e)=>setStatusAction(e.target.value)} className="w-full bg-[#0D1B2A] border border-white/10 text-white rounded-md py-3 px-4 outline-none focus:border-[#2DD4BF] text-xs font-mono">
                        <option>Pending Site Plan</option>
                        <option>Title Verified</option>
                        <option>Indenture Minted</option>
                      </select>
                    </div>
                    <button onClick={handleMilestoneUpdate} className="w-full py-4 bg-gradient-to-r from-[#2DD4BF] to-[#10B981] text-[#0D1B2A] font-head font-medium tracking-tight uppercase tracking-[0.2em] text-[10px] rounded-md hover:scale-105 transition-all shadow-xl mt-4">
               ...
               <div className="bg-[#162A3E] border border-white/5 rounded-[32px] p-10 shadow-xl">
                  ...
                  <div className="space-y-6">
                     {[
                        ...
                        <div key={i} className="bg-[#0D1B2A]/50 p-8 rounded-md border border-white/5 grid grid-cols-1 md:grid-cols-4 gap-8 items-center group hover:border-[#2DD4BF]/30 transition-all">
                           <div>
                           ...
                           <div className="flex justify-end gap-3 uppercase text-[10px] font-medium tracking-tight tracking-widest">
                              <button className="px-5 py-3 text-red-400 border border-red-500/20 rounded-md hover:bg-red-500/10">Reject</button>
                              <button onClick={() => alert("Developer Activated. WhatsApp Signal Dispatched.")} className="px-6 py-3 bg-[#10B981] text-[#0D1B2A] rounded-md shadow-lg">Activate Node</button>
                           </div>
                        </div>
                     ))}
                  </div>
             ...
             <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="bg-[#162A3E] p-10 rounded-[32px] border border-white/5">
                ...
                <div className="space-y-4">
                   {[
                      ...
                      <div key={i} className="flex justify-between items-center bg-[#0D1B2A]/50 p-6 rounded-md border border-white/5">
                         <div>
          ...
          {activeTab === 'the_node' && (
             <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-20}} className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <button className="px-12 py-5 bg-[#D4AF37] text-[#0D1B2A] font-head font-medium tracking-tight uppercase tracking-widest text-[10px] rounded-md shadow-2xl hover:scale-105 transition-all">Broadcast Handshake</button>
             </motion.div>
          )}
        </AnimatePresence>
      </main>
