      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        <header className="mb-16 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 border-b border-white/5 pb-12">
          <div>
            <div className="inline-flex items-center gap-2 mb-4 bg-[#D4AF37]/10 px-3 py-1 rounded-full border border-[#D4AF37]/20">
              ...
              <span className="text-[9px] font-bold uppercase tracking-[2px] text-[#D4AF37]">Premium Asset Audit</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-white mb-2">My Portfolio Performance</h1>
          ...
          <div className="flex gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/10 self-stretch md:self-auto">
             {['30 Days', '90 Days', '1 Year', 'All Time'].map(range => (
               <button 
                 ...
                 className={`flex-1 md:flex-none px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${timeRange === range ? 'bg-[#D4AF37] text-[#003300]' : 'hover:bg-white/5'}`}
               >
                 {range}
               </button>
             ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {kpis.map((kpi, i) => (
            <div key={i} className="bg-white/5 border border-white/10 p-8 rounded-3xl shadow-sm hover:border-[#D4AF37]/30 transition-all">
               <p className="text-[10px] font-bold uppercase tracking-[2px] opacity-40 mb-4">{kpi.label}</p>
               <h3 className="text-2xl font-bold text-white mb-1 tracking-tight">{kpi.value}</h3>
               <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-bold ${kpi.trend.startsWith('+') ? 'text-[#1D9E75]' : 'text-[#D4AF37]'}`}>{kpi.trend}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
           <div className="lg:col-span-2 bg-white/5 border border-white/10 p-10 rounded-[3rem] h-96 flex flex-col justify-between relative overflow-hidden group">
              ...
              <h4 className="text-sm font-bold uppercase tracking-widest opacity-40 mb-10">Portfolio Value Projection</h4>
              ...
              <p className="mt-8 text-[10px] font-bold uppercase tracking-widest text-[#D4AF37]">Aggregated Asset Appreciation Trend</p>
           </div>
           
           <div className="bg-[#003300] border border-[#D4AF37]/20 p-10 rounded-[3rem] flex flex-col justify-between">
              <h4 className="text-sm font-bold uppercase tracking-widest opacity-40 mb-10 text-[#F8F1E3]">Income Distribution</h4>
              <div className="flex-1 flex items-center justify-center relative">
                 ...
                 <div className="absolute flex flex-col items-center">
                    <p className="text-xl font-bold text-[#D4AF37]">72%</p>
                    <p className="text-[8px] font-bold uppercase opacity-60">Rent Yield</p>
                 </div>
              </div>
              <div className="space-y-3 mt-8">
                 <div className="flex justify-between text-[10px] font-bold uppercase"><span className="opacity-50">Residential</span> <span>GH₵12,700</span></div>
                 <div className="flex justify-between text-[10px] font-bold uppercase"><span className="opacity-50">Industrial</span> <span>GH₵15,750</span></div>
              </div>
           </div>
        </div>
        ...
        <section className="mb-20">
          <h2 className="text-2xl font-bold mb-10">Asset Performance Grid</h2>
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] overflow-hidden">
             <div className="overflow-x-auto">
               <table className="w-full text-left border-collapse">
                 <thead>
                    <tr className="bg-white/5 text-[10px] font-bold uppercase tracking-widest opacity-40">
                   ...
                   {propertyPerformance.map((p, i) => (
                      <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="px-10 py-8">
                           <p className="text-sm font-bold group-hover:text-[#D4AF37] transition-all">{p.name}</p>
                           <p className="text-[9px] font-bold opacity-30 uppercase tracking-widest">Sovereign Vault ID: SX-8299</p>
                        </td>
                        <td className="px-10 py-8 text-sm font-bold">{p.value}</td>
                        <td className="px-10 py-8">
                           <p className="text-sm font-bold text-[#D4AF37]">{p.apy}</p>
                           <p className="text-[9px] font-medium opacity-40 uppercase">Benchmark: +12.1%</p>
                        </td>
                        <td className="px-10 py-8">
                           <span className="text-[9px] font-bold bg-[#1D9E75]/10 text-[#1D9E75] px-3 py-1.5 rounded-full uppercase tracking-tighter border border-[#1D9E75]/20">
                             {p.status}
                           </span>
                        </td>
                      </tr>
        ...
        <section className="bg-[#D4AF37] text-[#003300] p-12 md:p-16 rounded-[4rem] flex flex-col md:flex-row items-center justify-between gap-12 shadow-2xl">
           <div className="max-w-xl text-center md:text-left">
             <h2 className="text-3xl font-bold mb-4 italic">Performance Highlights</h2>
             <p className="font-bold text-sm mb-2">Your portfolio is outperforming the market by 9.4%.</p>
