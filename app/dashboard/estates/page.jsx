    ...
    <div className="bg-syntry-obsidian min-h-screen text-white font-sans">
      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        <section className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-medium mb-4 tracking-tight">Welcome to Syntry Estates</h1>
          <p className="text-lg text-syntry-slate/70">Affordable, Fast Homes Backed by Syntry 8-Layer Verification.</p>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {activeProjects.map((proj) => (
              <div key={proj.id} className="bg-white/5 border border-white/10 p-8 rounded-3xl shadow-sm hover:shadow-xl transition-all">
                ...
                <p className="text-xs text-syntry-slate/60 mb-6 font-medium">{proj.location} • ETA: {proj.eta}</p>
        ...
        <section className="mb-20 bg-syntry-teal/5 border border-syntry-teal/20 p-10 rounded-[3rem] text-center md:text-left flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {availableHomes.map((home) => (
              <div key={home.id} className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden group hover:shadow-2xl transition-all relative">
                ...
                <div className="p-8">
                  <h3 className="text-xl font-medium mb-2 group-hover:text-syntry-amber transition-all">{home.title}</h3>
        ...
        {/* Why Choose Syntry Estates */}
        <section className="mt-24 py-20 border-t border-white/10 grid grid-cols-1 md:grid-cols-4 gap-8">
           {[
             ...
             <div key={i} className="space-y-3">
               <div className="w-10 h-10 bg-syntry-teal/10 text-syntry-teal rounded-lg flex items-center justify-center font-medium">✓</div>
               <h4 className="font-medium">{feat.t}</h4>
               <p className="text-xs text-syntry-slate/60 leading-relaxed font-medium">{feat.d}</p>
             </div>
           ))}
        </section>
      </main>
...
export default SyntryEstatesDashboard;
