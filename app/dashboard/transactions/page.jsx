    ...
    <div className="bg-[#fdfcf9] min-h-screen text-[#003300] font-sans">
      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
        <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <h1 className="text-4xl font-bold mb-2 tracking-tight">My Transactions</h1>
            <p className="opacity-60 text-sm">Review your sovereign financial interactions and asset movements.</p>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
             <button className="flex-1 md:flex-none border border-[#003300]/10 px-6 py-3 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-[#003300]/5 transition-all">
               Export CSV
             </button>
             <div className="flex-1 md:w-64 relative">
               <input 
                 ...
                 className="w-full py-3 px-10 rounded-xl border border-[#003300]/10 text-xs"
               />
        ...
        {/* Filters */}
        <div className="flex gap-3 overflow-x-auto no-scrollbar mb-10 pb-2 border-b border-[#003300]/5">
          {['All', 'Mortgage', 'Rent', 'Escrow', 'Verification'].map((tab) => (
             <button 
               key={tab}
               onClick={() => setActiveTab(tab)}
               className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-[#003300] text-white shadow-lg' : 'bg-white border border-[#003300]/10 opacity-60 hover:opacity-100'}`}
             >
               {tab}
             </button>
          ))}
        ...
        <div className="space-y-4">
          {filteredTransactions.length > 0 ? (
            filteredTransactions.map((tx) => (
              <div key={tx.id} className="bg-white border border-[#D4AF37]/5 p-6 md:p-8 rounded-3xl shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row items-center justify-between gap-6 group">
                ...
                <div className="text-center md:text-right w-full md:w-40">
                   <p className="text-xs font-bold mb-2">{tx.amount}</p>
                   <span className={`text-[9px] font-bold uppercase px-3 py-1 rounded-full ${tx.status === 'Completed' ? 'bg-[#1D9E75]/10 text-[#1D9E75]' : tx.status === 'Failed' ? 'bg-red-50 text-red-500' : 'bg-orange-50 text-orange-500'}`}>
                     {tx.status}
                   </span>
                </div>
              </div>
            ...
            <div className="py-32 text-center opacity-40">
              ...
              <h3 className="text-xl font-bold italic mb-2">No transactions matching this filter.</h3>
