  ...
  return (
    <div className="bg-[#fdfcf9] min-h-screen text-[#0F172A] font-sans">
      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
        <header className="mb-8 border-b border-[#0F172A]/5 pb-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="hidden md:flex gap-4">
               <input 
                 ...
                 className="py-2.5 px-6 rounded-md border border-[#0F172A]/10 text-xs w-64 bg-white/50 focus:bg-white transition-all outline-none"
               />
            </div>
          </div>
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
            {['All', 'Mortgage Eligible', 'Verified Only'].map((filter) => (
              <button 
                key={filter}
                className="px-6 py-2 rounded-md text-[10px] font-medium uppercase tracking-widest border border-[#0F172A]/10 hover:border-[#D4AF37] hover:text-[#D4AF37] whitespace-nowrap transition-all"
              >
                {filter}
              </button>
            ))}
        ...
        {/* Summary Bar */}
        <div className="bg-syntry-obsidian text-[#F8F1E3] p-8 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-6 mb-12 shadow-xl border border-[#D4AF37]/20 relative overflow-hidden">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="w-12 h-12 bg-white/10 rounded-md flex items-center justify-center text-xl">💎</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
          {savedProperties.length > 0 ? (
            savedProperties.map((prop) => (
              <div key={prop.id} className="bg-white border border-[#D4AF37]/5 rounded-md overflow-hidden shadow-sm hover:shadow-xl transition-all relative group">
                <div className="h-56 bg-syntry-obsidian/5 flex items-center justify-center text-[10px] opacity-20 italic">
                  {prop.img}
                </div>
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  {prop.isVerified && (
                    <span className="bg-[#D4AF37] text-[#0F172A] text-[9px] font-medium px-3 py-1 rounded-md uppercase tracking-tighter shadow-sm border border-white/20">
                      Verified Sovereign
                    </span>
                  )}
                  {prop.mortgageEligible && (
                    <span className="bg-[#1D9E75] text-white text-[9px] font-medium px-3 py-1 rounded-md uppercase tracking-tighter shadow-sm border border-white/20">
                      Mortgage Eligible
                    </span>
                  )}
                </div>
                ...
                <div className="p-8 pb-10">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    <a href={`/marketplace/${prop.id}`} className="bg-syntry-obsidian text-white py-4 rounded-md font-medium text-xs text-center hover:scale-[1.02] transition-all shadow-lg hover:shadow-[#D4AF37]/10">
                      View Details
                    </a>
            ...
            <div className="col-span-full py-40 text-center opacity-40 border-2 border-dashed border-[#0F172A]/5 rounded-[3rem]">
              ...
              <a href="/marketplace" className="inline-block bg-[#D4AF37] text-white px-8 py-3 rounded-md font-medium text-xs uppercase tracking-widest hover:scale-105 transition-all">
                Browse Marketplace
              </a>
            </div>
          )}
        ...
        {/* CTA Bar */}
        <section className="bg-syntry-obsidian text-[#F8F1E3] p-10 rounded-[3rem] text-center shadow-2xl relative overflow-hidden">
           ...
           <a href="/marketplace" className="inline-block bg-[#00BFFF] text-[#0F172A] px-10 py-4 rounded-md font-medium hover:scale-105 transition-all text-sm relative z-10">
