    ...
    <div className="bg-[#F8F1E3] min-h-screen text-[#003300] font-sans">
      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-4xl mx-auto">
        <header className="flex justify-between items-end mb-12">
          <div>
            <h1 className="text-4xl font-bold mb-2 tracking-tight">Notifications</h1>
            <p className="text-sm opacity-60">Stay synchronized with your sovereign portfolio updates.</p>
          </div>
          <button className="text-[10px] font-bold uppercase tracking-widest text-[#00BFFF] hover:underline">
            Mark all as read
          </button>
        </header>

        {/* Filter Tabs */}
        <div className="flex gap-6 border-b border-[#003300]/10 mb-8 overflow-x-auto no-scrollbar">
          {['All', 'Mortgage', 'Property', 'System'].map((tab) => (
            <button 
              key={tab}
              onClick={() => setFilter(tab)}
              className={`pb-4 text-xs font-bold uppercase tracking-widest relative whitespace-nowrap ${filter === tab ? 'text-[#003300]' : 'opacity-40'}`}
            >
              {tab}
          ...
          {filtered.length > 0 ? (
            filtered.map((note) => (
              <div key={note.id} className={`bg-white border p-6 rounded-2xl shadow-sm flex gap-6 items-start transition-all ${note.isNew ? 'border-[#1D9E75]/30 bg-[#1D9E75]/5' : 'border-[#D4AF37]/10'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${note.isNew ? 'bg-[#1D9E75] text-white' : 'bg-[#003300]/5 text-[#003300]'}`}>
                  {note.type === 'Mortgage' && '💳'}
                  {note.type === 'Property' && '🏠'}
                  {note.type === 'System' && '⚙️'}
                </div>
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className="text-[10px] opacity-40 font-bold uppercase">{note.time}</span>
                  </div>
                  <p className="text-sm opacity-70 leading-relaxed mb-4">{note.message}</p>
                  {note.action && (
                    <button className="text-[#00BFFF] text-xs font-bold uppercase tracking-widest hover:underline flex items-center gap-2">
                      {note.action} ↗
                    </button>
                  )}
                </div>
            ...
            <div className="py-20 text-center space-y-4">
              ...
              <p className="text-lg font-bold opacity-40 italic">You're all caught up!</p>
              <a href="/marketplace" className="text-[#00BFFF] text-xs font-bold uppercase tracking-widest inline-block">
                Browse Marketplace
              </a>
            </div>
          )}
