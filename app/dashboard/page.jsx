    ...
    <div className="bg-[#001a00] min-h-screen text-[#F8F1E3] font-sans">
      ...
      <main className="pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        <section className="mb-12">
          <h1 className="text-3xl md:text-4xl font-bold mb-2">Good morning, {user.name}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {kpis.map((kpi, i) => (
            <div key={i} className="bg-[#003300] border border-[#D4AF37]/20 p-6 rounded-xl shadow-lg hover:shadow-[#D4AF37]/10 transition-all">
              <p className="text-xs opacity-60 uppercase mb-2">{kpi.label}</p>
              <h3 className="text-2xl font-bold text-[#D4AF37] mb-1">{kpi.value}</h3>
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-4 justify-between items-start">
            {[1, 2, 3, 4, 5, 6].map((step) => (
              <div key={step} className={`flex-1 min-w-[120px] text-center ${step <= 3 ? 'opacity-100' : 'opacity-40'}`}>
                <div className={`w-10 h-10 mx-auto rounded-full flex items-center justify-center mb-3 border ${step <= 3 ? 'bg-[#D4AF37] border-[#D4AF37] text-[#003300]' : 'border-[#F8F1E3]/20'}`}>
                  {step <= 3 ? '✓' : step}
                </div>
        ...
        <section>
          <div className="flex justify-between items-end mb-8">
            <h2 className="text-2xl font-bold">My Property Portfolio</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {properties.map((prop) => (
              <div key={prop.id} className="bg-[#002200] rounded-xl overflow-hidden border border-[#D4AF37]/10 group hover:border-[#D4AF37]/30 transition-all">
                <div className="h-48 bg-[#003300] flex items-center justify-center text-xs opacity-40">
                  {prop.img}
                </div>
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <h4 className="font-bold text-lg">{prop.title}</h4>
                    <span className="bg-[#D4AF37] text-[#003300] text-[10px] px-2 py-1 rounded font-bold uppercase">
                      Verified
                    </span>
                  </div>
                  <p className="text-2xl font-bold text-[#D4AF37] mb-4">GH₵{prop.price}</p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button className="bg-[#00BFFF] text-[#003300] py-2 rounded font-bold text-xs hover:bg-[#0099CC] transition-all">
                      Details
                    </button>
                    <button className="border border-[#F8F1E3]/20 py-2 rounded font-bold text-xs hover:bg-[#F8F1E3]/5 transition-all">
                      Report
                    </button>
                  </div>
                </div>
        ...
        <section className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-6">
          <button className="p-6 bg-[#D4AF37] text-[#003300] rounded-xl font-bold hover:scale-[1.02] transition-all text-left">
            <p className="text-xs uppercase mb-1">Action Required</p>
            <h4 className="text-lg">Upload Bank Statements</h4>
          </button>
          <button className="p-6 bg-[#003300] border border-[#00BFFF]/20 rounded-xl font-bold hover:scale-[1.02] transition-all text-left">
            <p className="text-xs uppercase mb-1 text-[#00BFFF]">Next Step</p>
            <h4 className="text-lg">View Pre-Approval Letter</h4>
          </button>
          <a href="https://wa.me/233531102292" target="_blank" rel="noreferrer" className="p-6 bg-[#25D366] text-white rounded-xl font-bold hover:scale-[1.02] transition-all text-left flex items-center justify-between">
            <div>
