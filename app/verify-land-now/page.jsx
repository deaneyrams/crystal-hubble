              ...
              <section id="results-dashboard" className="py-24 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                <div className="bg-white border-t-8 border-t-[#00C853] border border-slate-200 rounded-[3rem] p-8 md:p-12 shadow-xl relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-12">
                    <div className="space-y-6">
                       <div className="w-full h-64 bg-slate-100 rounded-[2.5rem] overflow-hidden border border-slate-200 relative shadow-inner">
                          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                            <div className="w-8 h-8 bg-[#00C853] rounded-full border-4 border-white shadow-xl animate-bounce"></div>
                          </div>
                       </div>
                       <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-4">
                          <div><p className="text-[10px] text-slate-400 font-black uppercase mb-1">Coordinates</p><p className="text-sm font-bold font-mono">{coordinates ? `${coordinates.lat}, ${coordinates.lng}` : '5.6037° N, -0.1870° W'}</p></div>
                          <div><p className="text-[10px] text-slate-400 font-black uppercase mb-1">Digital Address</p><p className="text-sm font-bold bg-white border border-slate-200 px-2 py-1 rounded inline-block">GA-183-8164</p></div>
                       </div>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-8 text-slate-900">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                          <p className="text-[10px] text-slate-400 font-black uppercase mb-6 border-b pb-2">Chain of Trust</p>
                          <div className="space-y-6 relative border-l-2 border-slate-100 pl-6 ml-2 text-slate-900">
                             {[{date:'Oct 21',e:'Transfer'},{date:'Mar 19',e:'Registration'}].map((item, i) => (
                               <div key={i} className="relative">
                                  <div className="absolute -left-[31px] top-1 w-3 h-3 rounded-full bg-white border-2 border-[#00C853]"></div>
                        ...
                        <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl flex flex-col justify-between">
                           <div>
                             <p className="text-[10px] text-[#00C853] font-black uppercase tracking-widest mb-1">Valuation</p>
                             <p className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#00C853] to-emerald-300">GH₵ 3.2M</p>
                           </div>
                           <p className="text-xl font-bold mt-4">12.4% <span className="text-xs text-slate-500">APY Yield</span></p>
                        </div>
                      </div>
                      ...
                      <div className="pt-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                           {['GPS Lock', 'Title ID', 'Nii Auth', 'Registry', 'Stool Land', 'Survey Map', 'Consent', 'Sovereign'].map(layer => (
                             <div key={layer} className="bg-slate-50 border border-slate-100 p-4 rounded-2xl flex flex-col items-center transition-all hover:bg-white text-slate-900">
                                <span className="text-[#00C853] font-black text-xl mb-1">✓</span>
                                <span className="text-[9px] font-black uppercase">{layer}</span>
                             </div>
                           ))}
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-12 border-t border-slate-100">
                    <button className="bg-slate-900 text-white px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl">Download PDF Report</button>
                    <button className="bg-white border-2 border-slate-200 text-slate-900 px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest">Share to WhatsApp</button>
                    <button className="bg-[#00C853] text-white px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg">🚀 List Property</button>
                  </div>
                </div>
              </section>
            </div>
          ...
          <section className="py-24 border-t border-slate-100 bg-slate-50/30">
             <div className="max-w-4xl mx-auto px-4 text-center">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                   <button className="w-full sm:w-auto bg-[#00C853] text-white px-10 py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Get Full Report</button>
                   <button className="w-full sm:w-auto bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">Talk to Expert 053 110 2292</button>
                </div>
             </div>
          </section>
        </main>
              ...
              <div className="bg-white w-[340px] md:w-[400px] rounded-[2.5rem] shadow-2xl border border-slate-200 mb-6 overflow-hidden animate-in slide-in-from-bottom-6">
                 <div className="bg-slate-900 text-white p-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       ...
                       <div><p className="font-black text-xs">Syntry AI</p><p className="text-[9px] text-[#00C853] uppercase font-black">Oracle Active</p></div>
                    </div>
